...
ELClientPacket* ELClient::protoCompletedCb(void) {
	...
	if (_debugEn) {
		_debug->print("ELC: got %u @%X: %X %X %X", _proto.dataLen,
				(uint32_t) _proto.buf, packet->cmd, packet->value,
				packet->argc);
		for (uint16_t i = 8; i < _proto.dataLen; i++) {
			_debug->print(" %X", *(uint8_t*) (_proto.buf + i));
		}
		_debug->print("\r\n");
	}
	...
	if (packet->cmd == CMD_RESP_V) {
		// value response
		_debug->print("RESP_V: %u\r\n", packet->value);
		return packet;
	} else if (packet->cmd == CMD_RESP_CB) {
		FP<void, void*> *fp;
		// callback reponse
		_debug->print("RESP_CB: %u %u", packet->value, packet->argc);
	...
	while (_serial->available()) {
		...
		if (value == SLIP_ESC) {
			...
			ELClientPacket *packet =
					_proto.dataLen >= 8 ? protoCompletedCb() : 0;
			_proto.dataLen = 0;
			_proto.isEsc = 0;
			if (packet != NULL)
				return packet;
...
uint16_t ELClient::crc16Data(const unsigned char *data, uint16_t len,
		uint16_t acc) {
