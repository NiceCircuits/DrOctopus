...
void ELClientMqtt::setup(void) {
	_elc->_debug->print("ConnectedCB is 0x%X", (uint32_t) &connectedCb);
...
void ELClientMqtt::lwt(const char* topic, const char* message, uint8_t qos,
		uint8_t retain) {
...
void ELClientMqtt::publish(const char* topic, const uint8_t* data,
...
void ELClientMqtt::publish(const char* topic, const char* data, uint8_t qos,
		uint8_t retain) {
