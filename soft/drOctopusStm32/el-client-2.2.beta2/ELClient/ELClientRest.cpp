...
void ELClientRest::restCallback(void *res) {
	...
	_elc->_debug->print("REST code %U", _status);
	_len = resp->popArgPtr(&_data);
}

int ELClientRest::begin(const char* host, uint16_t port, bool security) {
...
void ELClientRest::request(const char* path, const char* method,
		const char* data, int len) {
	_status = 0;
	if (remote_instance < 0)
		return;
	if (data != 0 && len > 0)
		_elc->Request(CMD_REST_REQUEST, remote_instance, 3);
	else
		_elc->Request(CMD_REST_REQUEST, remote_instance, 2);
